...
public class BaseDdlTranslator extends BaseTranslator<List<DdlModel>> {
	 ...
	 * @return
	 */
	protected String getSqlColumnStart() {
		return "\"";
	}
	 ...
	 * @return
	 */
	protected String getSqlColumnEnd() {
		return "\"";
	}
	...
	protected String genFullTableName(DdlModel model) {
		...
		if(!StringUtility.isNullOrEmpty(model.getNameSpace()) && !"-".equals(model.getNameSpace())) {
			space = String.format("%s%s%s.", this.getSqlColumnStart(), model.getNameSpace(), this.getSqlColumnEnd());
		}
		return String.format("%s%s%s%s", space, this.getSqlColumnStart(), model.getName(), this.getSqlColumnEnd());
	}
}
